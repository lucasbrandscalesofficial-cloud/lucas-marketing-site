...
const PainPointsSection = () => {
  ...
  return (
    <section className="section-pain section-block relative overflow-hidden">
      ...
      <div
        ref={ref}
        className={`relative mx-auto max-w-6xl transition-all duration-[4600ms] ease-[cubic-bezier(0.22,1,0.36,1)] ${visible ? "opacity-100 translate-y-0 blur-0" : "opacity-0 translate-y-3 blur-[2px]"}`}
      >
        <div className="grid items-start gap-8 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.1fr)] lg:gap-12">
          ...
          <ul className="grid gap-2.5 sm:grid-cols-2">
            {painPoints.map((point, i) => (
              <li
                key={point.title}
                className={`flex flex-col rounded-2xl border border-destructive/30 bg-white/95 p-4 shadow-lg shadow-destructive/[0.08] ring-1 ring-destructive/10 transition-all duration-[3900ms] ease-[cubic-bezier(0.22,1,0.36,1)] hover:-translate-y-0.5 hover:shadow-xl dark:bg-card/95 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-2"}`}
